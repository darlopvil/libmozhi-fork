...
// ============================================================================
// OPENROUTER Gemma — isla autónoma. Modelo fijo: google/gemma-4-31b-it:free
// ============================================================================

func openrouterGemmaChat(key, model, systemPrompt, userText string, temperature float64) (string, error) {
...
func openrouterGemmaStripFences(s string) string {
...
func translateOpenrouterGemma(to string, from string, text string) (LangOut, error) {
	FromOrig := from
	ToOrig := to
	if err := validateLanguagePair(langListOpenrouterGemma("sl"), langListOpenrouterGemma("tl"), from, to); err != nil {
		return LangOut{}, err
	}
	var langout LangOut
	langout.Engine = "gemma"
	...
	model := os.Getenv("MOZHI_OPENROUTER_GEMMA_MODEL")
	if model == "" {
		model = "google/gemma-4-31b-it:free"
	}

	targetName := to
	for _, l := range langListOpenrouterGemma("tl") {
		if l.Id == to {
			targetName = l.Name
			break
		}
	...
	if from != "auto" {
		for _, l := range langListOpenrouterGemma("sl") {
			if l.Id == from {
				srcName = l.Name
				break
			}
	...
	translated, err := openrouterGemmaChat(key, model, transSystem, text, 0.3)
	if err != nil {
		return LangOut{}, err
	}
	...
	if len(strings.Fields(text)) <= 8 {
		...
		if raw, derr := openrouterGemmaChat(key, model, dictSystem, text, 0.3); derr == nil {
			raw = openrouterGemmaStripFences(raw)
			if gjson.Valid(raw) {
