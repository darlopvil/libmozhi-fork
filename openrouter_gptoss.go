...
// ============================================================================
// OPENROUTER Gptoss — isla autónoma. Modelo fijo: openai/gpt-oss-20b:free
// ============================================================================

func openrouterGptossChat(key, model, systemPrompt, userText string, temperature float64) (string, error) {
...
func openrouterGptossStripFences(s string) string {
...
func translateOpenrouterGptoss(to string, from string, text string) (LangOut, error) {
	FromOrig := from
	ToOrig := to
	if err := validateLanguagePair(langListOpenrouterGptoss("sl"), langListOpenrouterGptoss("tl"), from, to); err != nil {
		return LangOut{}, err
	}
	var langout LangOut
	langout.Engine = "gptoss"
	...
	model := os.Getenv("MOZHI_OPENROUTER_GPTOSS_MODEL")
	if model == "" {
		model = "openai/gpt-oss-20b:free"
	}

	targetName := to
	for _, l := range langListOpenrouterGptoss("tl") {
		if l.Id == to {
			targetName = l.Name
			break
		}
	...
	if from != "auto" {
		for _, l := range langListOpenrouterGptoss("sl") {
			if l.Id == from {
				srcName = l.Name
				break
			}
	...
	translated, err := openrouterGptossChat(key, model, transSystem, text, 0.3)
	if err != nil {
		return LangOut{}, err
	}
	...
	if len(strings.Fields(text)) <= 8 {
		...
		if raw, derr := openrouterGptossChat(key, model, dictSystem, text, 0.3); derr == nil {
			raw = openrouterGptossStripFences(raw)
			if gjson.Valid(raw) {
